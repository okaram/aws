...
For S3, the client interface is `AmazonS3` and the client class is http://docs.aws.amazon.com/AWSJavaSDK/latest/javadoc/com/amazonaws/services/s3/AmazonS3Client.html[`AmazonS3Client`] (there's also an `AmazonS3ClientAsync`, with more capabilities). We create a `GetObjectRequest', specifying the bucket and the file name, and then  call `getObject` on the client.
