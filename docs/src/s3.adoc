...
For S3, the client interface is `AmazonS3` and the client class is `AmazonS3Client` (there's also an `AmazonS3ClientAsync`, with more capabilities). We create a `GetObjectRequest', specifying the bucket and the file name, and then  call `getObject` on the client.
